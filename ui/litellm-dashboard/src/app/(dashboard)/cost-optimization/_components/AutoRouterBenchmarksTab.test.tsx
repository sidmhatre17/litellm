...
describe("AutoRouterBenchmarksTab", () => {
  it("leads with estimated savings, before the three session-shape tiles", async () => {
    ...
    const labels = screen
      .getAllByText(/Estimated savings|Avg turns per session|Avg session length|Avg tokens per session/)
  ...
  it("renders the headline numbers the tiles exist for", async () => {
    ...
    expect(screen.getByText(/\$359\.86 routed vs \$2,534\.45 all-claude-opus-4-8/)).toBeInTheDocument();
  });
