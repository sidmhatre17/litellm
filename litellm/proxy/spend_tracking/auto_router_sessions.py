...
class TurnDelta:
    ...
    Every field is additive so the flusher can hand them to an atomic increment
    upsert, which is what lets two pods writing the same session compose instead
...
def merge_deltas(left: TurnDelta, right: TurnDelta) -> TurnDelta:
    ...
    Counters add and the later state wins, which is exactly what the database
    would have done had the two turns been flushed separately; folding them in
    memory first just spares the round trip.
    """
    return TurnDelta(
        turns=left.turns + right.turns,
        turns_with_usage=left.turns_with_usage + right.turns_with_usage,
        prompt_tokens=left.prompt_tokens + right.prompt_tokens,
        completion_tokens=left.completion_tokens + right.completion_tokens,
        total_tokens=left.total_tokens + right.total_tokens,
        ephemeral_5m_tokens=left.ephemeral_5m_tokens + right.ephemeral_5m_tokens,
        ephemeral_1h_tokens=left.ephemeral_1h_tokens + right.ephemeral_1h_tokens,
        spend=left.spend + right.spend,
        baseline_spend=left.baseline_spend + right.baseline_spend,
        same_model_turns=left.same_model_turns + right.same_model_turns,
        same_model_hits=left.same_model_hits + right.same_model_hits,
        first_visit_turns=left.first_visit_turns + right.first_visit_turns,
        first_visit_hits=left.first_visit_hits + right.first_visit_hits,
        return_turns=left.return_turns + right.return_turns,
        return_hits=left.return_hits + right.return_hits,
        stale_return_misses=left.stale_return_misses + right.stale_return_misses,
        savable_return_misses=left.savable_return_misses + right.savable_return_misses,
        rescued_spend=left.rescued_spend + right.rescued_spend,
        replay_spend=left.replay_spend + right.replay_spend,
        state=right.state,
    )
